function test_periodic_orbit(x0poTarget)

% TEST_PERIODIC_ORBIT functions integrates the periodic orbit initial
% condition for the time span [0 T] where T is the time period for the
% orbit.
...
for k = nFam:-1:1,
  ...
  tI = 0 ;
  tF = TPOFam(k,1) ;
  [x,t] = trajGet_ball_rolling(x0,tI,tF,1,OPTIONS) ;
  plot3(x(:,1),x(:,2),x(:,3),'.-');
  hold on
  grid on
  plot3(x0po(k,1), x0po(k,2), x0po(k,3), 'xb', ...
    x(end,1), x(end,2), x(end,3), 'ob');
  pause(0.5);
end
